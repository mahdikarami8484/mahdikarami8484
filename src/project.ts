    ...
    return `
      <div class="min-h-48 bg-white shadow shadow-gray-200 rounded-md py-4 px-5 flex flex-col gap-3 max-w-96 sm:w-72 w-96 md:w-96 min-w-72">
        ...
        <div class="flex mt-auto justify-around p-2 w-full gap-6">
          ${project.sourceLink ? `<a href="${project.sourceLink}" class="shadow shadow-gray-200 p-2 rounded-md text-gray-700 cursor-pointer hover:scale-105 hover:font-medium">Source</a>` : ""}
          ${project.liveLink ? `<a href="${project.liveLink}" class="shadow shadow-teal-200 p-2 rounded-md text-white cursor-pointer bg-teal-300 hover:scale-105 hover:font-medium">Live View</a>` : ""}
        </div>
      </div>
    `;
  }).join("\n");
